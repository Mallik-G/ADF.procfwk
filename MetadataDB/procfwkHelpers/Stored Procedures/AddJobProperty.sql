CREATE PROCEDURE [procfwkHelpers].[AddJobProperty]
	(
	@JobId INT,
...
BEGIN
	...
	IF EXISTS
		(
		SELECT * FROM [procfwk].[JobProperties] WHERE [JobId] = @JobId AND [PropertyName] = @PropertyName AND [ValidTo] IS NOT NULL
		)
		AND NOT EXISTS
		(
	...
	INSERT INTO [procfwk].[JobProperties]
		(
		[JobId],
		[PropertyName],
		[PropertyValue],
	...
	FROM
		(
		MERGE INTO
			[procfwk].[JobProperties] targetTable
		USING
			sourceTable
				ON sourceTable.[JobId] = targetTable.[JobId]
				AND sourceTable.[PropertyName] = targetTable.[PropertyName]	
